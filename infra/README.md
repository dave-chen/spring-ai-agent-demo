...
You may also pass `AGENT_OIDC_PROVIDER_DOMAIN=token.actions.githubusercontent.com` (preferred) or `AGENT_OIDC_PROVIDER_ARN=arn:aws:iam::123456789012:oidc-provider/token.actions.githubusercontent.com`. When you pass `AGENT_OIDC_PROVIDER_DOMAIN` the deployment template will use the account id to build an ARN safely without relying on manual substitution.
```
